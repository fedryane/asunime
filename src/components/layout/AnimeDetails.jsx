    ...
    <div className="w-full h-[500px]">
      <div className="relative">
        <div className="flex flex-wrap justify-between ">
          <div className="w-[100%] lg:w-[70%] h-[500px]  flex flex-col lg:flex-row justify-center items-center lg:items-stretch lg:items-none lg:justify-start p-10 gap-5">
            <img
              src={detail.image}
              alt=""
              className="w-[185px] z-[0] lg:w-[300px] lg:h-[400px] px-5"
            />
            <div className="flex flex-col text-center lg:text-start px-5">
              <h1 className="text-white font-normal text-[25px] lg:text-[27px]">
                {detail.title.romaji}
              </h1>
                  ...
                  <>
                    {!show ? (
                      <button
                        ...
                        className="text-gray-200 hover:text-gray-400 underline flex items-center text-[12px] lg:text-[13px]"
                      >
                        <AiOutlinePlus />
                        More
                      </button>
          ...
          <div className="w-[100%] h-[570px] lg:w-[25%] lg:h-[550px]  flex  bg-[#5c5d5f] opacity-50">
            <div className="px-7 flex flex-col gap-2 text-[15px] mt-5 lg:mt-16 font-extrabold opacity-100">
              <div className="text-white block lg:hidden">
                Overview :
                <p className="font-medium text-white overflow-y-auto h-32">
                  {" "}
                  {detail.description}
                </p>
              </div>
              ...
              <p className="text-white">
                ...
                <span className="font-medium">
                  {" "}
                  {detail.synonyms.map((i) => i)}
                </span>
              </p>
              <p className="text-white">
                Total Episode :
                <span className="font-medium"> {detail.totalEpisodes}</span>
              </p>
              <p className="text-white">
              ...
              <p className="text-white">
                ...
                <span className="font-medium">
                  {" "}
                  {detail?.season?.toLowerCase().charAt(0).toUpperCase() +
                    detail?.season?.toLowerCase().slice(1)}{" "}
                  {detail.releaseDate} Anime
                </span>
              </p>
              <p className="text-white">
                Studio :
                <span className="font-medium">
                  {" "}
                  {detail.studios.map((studio) => studio)}
                </span>
              </p>
              <p className="text-white">
                Score :{" "}
                <span className="font-medium border p-1">{detail.rating}</span>
              </p>
              <p className="text-white">
                Start Date :{" "}
                <span className="font-medium">
                  {detail.startDate.day}-{detail.startDate.month}-
              ...
              <p className="text-white">
                End Date :{" "}
                <span className="font-medium">
                  {detail.endDate.day}-{detail.endDate.month}-
                  {detail.endDate.year}
                </span>
              </p>
              <p className="text-white">
        ...
        <img
          ...
          loading="lazy"
          alt=""
          className="absolute z-[-1] top-0 left-0 right-0 w-full h-[549px] blur-lg opacity-80"
        />
      </div>
                  ...
                  return (
                    ...
                    >
                      <LazyLoadImage
                        src={i.image}
                        alt=""
                        placeholderSrc={i.image}
                        effect="blur"
                        loading="lazy"
                        className="w-32 h-40 lg:w-40 lg:h-56 rounded-lg"
                      />
                      <div className="text-gray-300 text-center mt-3">
                        <p className="font-bold">
                          {i.name.full.length > 12 ? i.name.first : i.name.full}
                        </p>
                        <p
                          className={`${
                            i.role === "MAIN"
                        ...
                        >
                          {i.role.toLowerCase().charAt(0).toUpperCase() +
                            i.role.toLowerCase().slice(1)}
                        </p>
                      </div>
                    </div>
                  );
